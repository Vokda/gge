...
Square::Square(Square_coordinate sc, const SDL_Point& center_point, double size, SDL_Color c ): 
	Tile(center_point, size, c),
	_square_coordinate(sc)
{
	calculate_corners(center_point, size);
	_log_stream << "Created square " << *this;
	_log_stream.flush();
}
